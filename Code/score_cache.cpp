...
void scoring::ScoreCache::read(std::string filename) {
    ...
    while (!in.eof()) {
        ...
        boost::trim(tokens[0]);
        boost::trim(tokens[1]);
        if(tokens[0]=="num_records"){
            std::string tempnum = tokens[1];
            num_records = std::atoi(tempnum.c_str());
        }
    ...
    threshold = num_records/1000 + 0.5;//0.5 is used to correct the threshold coefficient of 0

    //The threshold can be set according to the number of variables during the test time
    // if(variableCount >= 25) threshold = 0.09;
    // else if(variableCount > 20)threshold = 0.4-(variableCount-20)*0.05;
    //     else if(variableCount == 20) threshold = 2;

    //Set the threshold to 0 when testing memory
    //threshold = 0.0; 

    //caculate score prepare
    int large = 0;
    in.open(filename.c_str());
    while (!in.eof()) {
        std::getline(in, line);
        
        if (line.size() == 0 || line.compare(0, 1, "#") == 0 || contains(line, "meta") || contains(line, "var ")) {
            continue;
        }
        tokens = parse(line, 0, " ");
        float score = -1 * atof(tokens[0].c_str()); // multiply by -1 to minimize
        if(large == 0 && score > 1000.0) {large = 1;break;}
    }
    in.close();
    ...
    while (!in.eof()) {
        ...
        float score = -1 * atof(tokens[0].c_str()); // multiply by -1 to minimize
        if(large == 1) score *= 1.25;//Fractional correction, so that the value in the appropriate range
...
void scoring::ScoreCache::readUrlBinary(std::string filename) {
    ...
    for (int variable = 0; variable < variableCount; variable++) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            VARSET_NEW_INIT(parents, variableCount_, l);
